...
// on, and only once per class. Use `idevicesyslog | grep ClockCustomizer`
// to read the output and find the real label ivar names on your build.
...
- (void)didMoveToWindow {
    ...
    if (self.window == nil) {
        // View left the window (e.g. device locked screen dismissed) — stop our timer.
    ...
    if (!timeLabel) {
        DebugLog(@"Could not find time label on %@ — enable Debug Logging and check syslog for ivar dump.", NSStringFromClass([self class]));
        return;
    }

    // Apply custom font, preserving the original point size.
    ...
    void (^tick)(void) = ^{
        ...
        // Respect the user's 12/24 hour system preference where possible.
    ...
    // When seconds are off we leave Apple's own updating in place (%orig already ran).
}
...
// ---------------------------------------------------------------------------
// FINDING THE RIGHT CLASS on your exact 17.0–17.3 build, if the hook above
// silently does nothing:
//
//   1. Enable "Debug Logging" in Settings > ClockCustomizer, respring.
//   2. `idevicesyslog | grep ClockCustomizer` while unlocking your device.
//   3. If you see no "---- Ivars ----" lines at all, CSLockScreenLiveClockView
//      isn't the right class name on your build. Grab a class-dump of
//      SpringBoard (via a dumped dyld_shared_cache) and grep for "Clock".
//      Rename the class in the %hook line above to whatever you find, e.g.
//      `%hook SBUILockScreenLiveClockView`.
//   4. If ivars are dumped but no label is found, add the printed name(s)
//      that look like the time label to kTimeLabelIvarCandidates above.
// ---------------------------------------------------------------------------
